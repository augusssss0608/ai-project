...
(function setupNewsRefresh(){
  ...
  btn.addEventListener('click', async (ev) => {
    ...
    btn.disabled = true;
    // Shift/Alt 点击 → 只抓取不生成摘要 (快速测试)
    const quick = ev.shiftKey || ev.altKey;
    const url = quick ? '/news/refresh?no_summary=1' : '/news/refresh';
    const waitMsg = quick ? '抓取中...' : '抓取+摘要生成中 (约 3-8 分钟)';
    btn.textContent = waitMsg;
    // 启动一个简单的经过秒数计时器
    const startedAt = Date.now();
    const timer = setInterval(() => {
      const s = Math.floor((Date.now() - startedAt) / 1000);
      btn.textContent = `${waitMsg} [${s}s]`;
    }, 1000);
    try {
      ...
      if (data.ok) {
        ...
        btn.textContent = '失败';
        console.warn('[news refresh]', data);
