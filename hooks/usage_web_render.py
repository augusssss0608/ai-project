...
def _render_news_item(parts: list, it: dict, source_id: str, voted_urls: set, hidden: bool = False):
    title = it.get("title", "").strip() or "(no title)"
    url = it.get("url", "") or "#"
    is_voted = url in voted_urls
    ...
    parts.append(
        ...
        f"title='{'已标记有帮助 (再点取消)' if is_voted else '标记为有帮助'}'>"
        f"{btn_label}</button>"
    )
    parts.append("</div>")
    meta_bits = []
    if "score" in it and it["score"]:
        meta_bits.append(f"▲ {it['score']}")
    if "comments" in it and it["comments"]:
        meta_bits.append(f"💬 {it['comments']}")
    if it.get("today_stars"):
        meta_bits.append(f"⭐ {html.escape(it['today_stars'])}")
    elif it.get("stars"):
        meta_bits.append(f"⭐ {html.escape(it['stars'])}")
    if it.get("lang"):
        meta_bits.append(html.escape(it["lang"]))
    if it.get("author"):
        meta_bits.append(f"@{html.escape(it['author'])}")
    if it.get("ts"):
        meta_bits.append(_fmt_news_ts(it["ts"]))
    ...
    if meta_bits:
        parts.append(f"<div class='news-item-meta'>{' · '.join(meta_bits)}</div>")
    # Opus 分析: 工作区帮助 + Claude 使用. 只有两者都是 '无相关' 时整块隐藏
    ws = (it.get("workspace_help") or "").strip()
    cu = (it.get("claude_usage") or "").strip()
    if ws or cu:
        ws_is_na = (not ws) or ws == "无相关"
        cu_is_na = (not cu) or cu == "无相关"
        if not (ws_is_na and cu_is_na):
            ...
            parts.append(
                ...
                f"<span class='news-analysis-text'>{html.escape(ws or '无相关')}</span>"
                f"</div>"
            )
            parts.append(
                f"<div class='news-analysis-row {'na' if cu_is_na else 'rel'}'>"
                f"<span class='news-analysis-label'>Claude</span>"
                f"<span class='news-analysis-text'>{html.escape(cu or '无相关')}</span>"
                f"</div>"
            )
            parts.append("</div>")
...
def _render_news_source_card(parts: list, src: dict, voted_urls: set):
    ...
    for i, it in enumerate(items):
        _render_news_item(parts, it, sid, voted_urls, hidden=(i >= NEWS_VISIBLE_COUNT))
    parts.append("</ol>")
...
def _render_news_panel(parts: list):
    ...
    votes = _load_news_votes()
    voted_urls = set(votes.keys())
    parts.append("<div class='section'>")
    ...
    for src in data.get("sources", []):
        _render_news_source_card(parts, src, voted_urls)
    parts.append("</div>")
    parts.append("</div>")
