...
**分 6 个 chunks** (新增 Chunk 0 迁移), 每 chunk 逻辑自包含:
- **Chunk 0: 项目迁移 + git init** — 从 `~/.claude/` 迁代码 + 数据到 `~/Desktop/ai-project/`, dashboard 原位重启 (MANUAL + AUTO 混合)
- Chunk 1: Python 基础模块 + fetcher.yaml 前置 (全 AUTO, 严格串行)
...
## Chunk 0: 项目迁移 + git init

**范围**: 把"统计系统"从 `~/.claude/hooks/` + `~/.claude/usage-stats/` 迁到 `~/Desktop/ai-project/`, dashboard 从新位置重启. 这是所有后续任务的前置.

**任务标签**: AUTO 为主, 2 处 MANUAL (停/启 dashboard 需要人工确认).

**前提**: /loop 启动前的人工准备 (plan 开头段) 已完成, 此时项目目录和 git 已存在, plan + spec 已 mv 进 docs/.

**文件迁移清单**:

从 `~/.claude/hooks/` 迁到 `~/Desktop/ai-project/hooks/`:
- usage-web.py, usage-web.js, usage-web.css
- usage_web_core.py, usage_web_queries.py, usage_web_render.py, usage_web_summary.py
- fetch-ai-news.py (稍后 Chunk 1 会重写)
- usage-tracker.sh, usage-archive-cold.sh, usage-rebuild-db.sh, usage-report.sh
- USAGE-STATS.md
- usage.db

从 `~/.claude/usage-stats/` 迁到 `~/Desktop/ai-project/data/`:
- events.db, events.jsonl
- ai-news.json, ai-news-feedback.json
- summaries.json, summary-quota.json
- archive-log.jsonl, tracker-errors.log

**留在 `~/.claude/hooks/` 不动**:
- pre-compact.sh, post-compact.sh, session-start.sh (Claude Code 机制 hook)
- __pycache__ / 各 .bak 文件 (忽略)

---

### Task 0.1: [MANUAL] 停 dashboard

**原因**: /loop 不能自己判断 dashboard 是否停好 (pkill 结果可能卡等, 或者 user 不希望此时停).

- [ ] **Step 1: /loop 写 NEED-HUMAN-INPUT.md**

```markdown
resolved: false

## Task 0.1: 停 dashboard 以便迁移文件

/loop 准备迁移 ~/Desktop/ai-project/hooks/usage-web.py 等文件, 需要你先停掉运行中的 dashboard 进程.

### 你要做的:
1. `pkill -f 'python3.*usage-web.py'` (或手动 kill)
2. 验证: `lsof -i :38080` 应无输出
3. 在本文件顶改 resolved: true
```

- [ ] **Step 2: ScheduleWakeup 挂起等 resolved**

- [ ] **Step 3: Mark complete (resolved 后)**

---

### Task 0.2: [AUTO] cp dashboard 代码 + 维护脚本到项目

**Files:**
- Source: `~/.claude/hooks/` 各文件
- Target: `~/Desktop/ai-project/hooks/`

- [ ] **Step 1: cp 全部统计系统相关文件**

```bash
cd ~/.claude/hooks
cp usage-web.py usage-web.js usage-web.css \
   usage_web_core.py usage_web_queries.py usage_web_render.py usage_web_summary.py \
   fetch-ai-news.py \
   usage-tracker.sh usage-archive-cold.sh usage-rebuild-db.sh usage-report.sh \
   USAGE-STATS.md usage.db \
   ~/Desktop/ai-project/hooks/
```

- [ ] **Step 2: 验证**

```bash
ls ~/Desktop/ai-project/hooks/
```
Expected: 上面 14 个文件都在.
...
cd ~/Desktop/ai-project
git add hooks/
git commit -m "chore(migrate): copy stats system code from ~/.claude/hooks/"
```
...
### Task 0.3: [AUTO] cp 数据到项目 data/

- [ ] **Step 1: cp 全部数据**

```bash
cd ~/.claude/usage-stats
cp events.db events.jsonl \
   ai-news.json ai-news-feedback.json \
   summaries.json summary-quota.json \
   archive-log.jsonl tracker-errors.log \
   ~/Desktop/ai-project/data/
```

若某文件不存在 (如 tracker-errors.log 可能空), 跳过该文件不报错.

- [ ] **Step 2: 验证**

```bash
ls -la ~/Desktop/ai-project/data/
```
Expected: events.db / events.jsonl / ai-news.json 等存在.

- [ ] **Step 3: 不 commit data/** (.gitignore 已排除). 但加个 `.gitkeep` 防目录丢失:

```bash
touch ~/Desktop/ai-project/data/.gitkeep
cd ~/Desktop/ai-project
git add data/.gitkeep
git commit -m "chore(migrate): keep data/ dir (gitignored content)"
```
...
### Task 0.4: [AUTO] 改硬编码路径常量

**Files:** 要改的 Python 文件 (在项目 hooks/ 下)

关键路径常量改动:
- `usage_web_core.py`: `NEWS_JSON_PATH`, `NEWS_VOTES_PATH`, `EVENTS_DB_PATH`, `SUMMARIES_PATH`, `SUMMARY_QUOTA_PATH`, `ARCHIVE_LOG_PATH`, `NEWS_FETCHER_PATH` 等
- `fetch-ai-news.py`: `OUT_PATH` (改到 `~/Desktop/ai-project/data/ai-news.json`)
- 任何其他硬编码 `~/.claude/usage-stats/` 或 `~/.claude/hooks/fetch-ai-news.py` 等原 `~/.claude/` 路径的地方

- [ ] **Step 1: Grep 全部硬编码路径**

```bash
cd ~/Desktop/ai-project/hooks
grep -rn 'usage-stats\|/.claude/hooks\|fetch-ai-news.py' *.py | head -50
```
记录所有出现的位置.
...
统一替换规则:
- `~/.claude/usage-stats/` → `~/Desktop/ai-project/data/`
- `~/.claude/hooks/fetch-ai-news.py` → `~/Desktop/ai-project/hooks/fetch-ai-news.py`
- `~/.claude/hooks/ai_news` → `~/Desktop/ai-project/hooks/ai_news` (为 Chunk 1 做准备, 即使 ai_news/ 这时还没建)

保留不改的:
- `~/.claude/skills/` 路径 (dashboard 的 archive 功能是对 user-level skill 操作, 不应改)
- `~/.claude/agents/` (同上)
...
```bash
cd ~/Desktop/ai-project/hooks
for f in *.py; do python3 -c "import ast; ast.parse(open('$f').read())" && echo "$f OK" || echo "$f FAIL"; done
```
Expected: 所有 OK.
...
```bash
cd ~/Desktop/ai-project/hooks
grep -rn 'usage-stats\|/.claude/hooks' *.py
```
Expected: 只剩 `~/.claude/skills/` 或 `~/.claude/agents/` 相关 (这些保留).
...
cd ~/Desktop/ai-project
git add hooks/
git commit -m "refactor(hooks): rewire paths to project data/ and hooks/"
```
...
### Task 0.5: [MANUAL] 启动新位置 dashboard + 浏览器验证所有 tab

**原因**: /loop 自己起 dashboard 并观察它能否跑, 但"所有 tab 都能正常显示数据"需要你人眼看.

- [ ] **Step 1: /loop 启动 dashboard (subprocess, 后台)**

```bash
python3 ~/Desktop/ai-project/hooks/usage-web.py &
sleep 3
curl -sS http://localhost:38080/ | head -30 > /tmp/dashboard-probe.html
```
Expected: HTML 返回不为空, 有 `<title>` 标签.

若 curl 失败, 说明 dashboard 启动有问题 (可能路径常量漏改). 看 `logs/task-0.5-dashboard.log` stderr.

- [ ] **Step 2: /loop 写 NEED-HUMAN-INPUT.md**

```markdown
resolved: false

## Task 0.5: 浏览器验证 dashboard 迁移后所有 tab 正常

/loop 已启动 ~/Desktop/ai-project/hooks/usage-web.py. curl 探测返回 200.

### 你要做的:
1. 浏览器打开 http://localhost:38080
2. 依次切到每个 tab (usage 统计 / skill 管理 / subagent 管理 / summary / 每日 AI 大事), 确认:
   - 数据能加载 (不是空 / 不是报错)
   - 点按钮功能正常 (如 vote / archive 等, 可试几个)
3. 若全部正常 → 本文件改 resolved: true, verdict: PASS
4. 若某 tab 挂 → resolved: true, verdict: FAIL, 写哪个 tab 挂了 + 错误信息
```

- [ ] **Step 3: 挂起等 resolved**

若 verdict: PASS → 进下一 task. 若 FAIL → /loop 把 blocked_on 标 AUTO_FAIL, 自己去看 log 修 (可能是 Task 0.4 改路径漏了某处).

- [ ] **Step 4: Mark complete**

---

### Task 0.6: [AUTO] 禁用 ~/.claude/hooks/ 下的旧 dashboard (防止误启)

- [ ] **Step 1: 移动旧文件到 .migrated 子目录 (保留备份 1 周)**

```bash
mkdir -p ~/.claude/hooks/.migrated-to-ai-project-20260421
cd ~/.claude/hooks
mv usage-web.py usage-web.js usage-web.css \
   usage_web_core.py usage_web_queries.py usage_web_render.py usage_web_summary.py \
   fetch-ai-news.py \
   usage-tracker.sh usage-archive-cold.sh usage-rebuild-db.sh usage-report.sh \
   USAGE-STATS.md usage.db \
   .migrated-to-ai-project-20260421/
```

**留在 `~/.claude/hooks/` 顶层**:
- pre-compact.sh, post-compact.sh, session-start.sh (Claude Code 机制 hook, 不动)
- __pycache__ / *.bak (不影响, 无需迁)
...
ls ~/.claude/hooks/
```
Expected: 只剩 3 个 .sh + __pycache__ + .bak 文件 + .migrated-to-ai-project-20260421/ 目录.

- [ ] **Step 3: 确认 /loop 本身所依赖的 hook 还在**

```bash
ls ~/.claude/hooks/{pre-compact.sh,post-compact.sh,session-start.sh}
```
Expected: 3 文件都存在.

- [ ] **Step 4: Mark complete**

**迁移完成说明**: 1 周后确认新位置稳定后, 用户可手动 `rm -rf ~/.claude/hooks/.migrated-to-ai-project-20260421/`. 本 plan 不做自动清理.

---

### Task 0.7: [AUTO] 数据可选清理 — ~/.claude/usage-stats/ 空化

**原因**: 数据已 cp 到 `~/Desktop/ai-project/data/`, 原位保留只是占空间. 但保留原目录 1 周作为备份.

- [ ] **Step 1: 检查原位数据仍完整**

```bash
ls ~/.claude/usage-stats/
```
Expected: 和 Task 0.3 cp 前一样 (仍有 events.db 等).

- [ ] **Step 2: 不删除 (保留备份)**

暂不操作. 1 周后用户若确认新位置稳定, 可手动 `rm -rf ~/.claude/usage-stats/*`.

- [ ] **Step 3: Mark complete** (这个 task 主要是 progress 记录"数据迁移已完成, 原位保留备份")

---

### Task 0.8: [AUTO] 项目 README + 初始 commit

**Files:** `~/Desktop/ai-project/README.md`
...
AI / 统计系统独立项目, 从 `~/.claude/hooks/` + `~/.claude/usage-stats/` 迁移.

## 目录结构

- `.claude/skills/` — 项目级 skill (ai-news-fetch, ai-news-filter)
- `.claude/agents/` — 项目级 subagent (news-scorer, news-summary, news-analysis, evolve-source-preferences)
- `hooks/` — Python 代码 (dashboard + ai_news pipeline)
- `data/` — 数据文件 (events.db / ai-news.json 等, gitignore)
- `docs/` — spec + plan
...
claude --model sonnet
/loop
# prompt: 使用 ai-news-fetch skill, 到 10:00 跑 pipeline
\`\`\`
...
claude --model sonnet
/loop
# prompt: 按 docs/plan.md 执行
\`\`\`

## 文件路径

全部绝对路径 `~/Desktop/ai-project/...`, 见 docs/plan.md "项目结构" 段.
```
...
- [ ] **Step 3: Mark complete** (Chunk 0 完成)
...
- **Chunk 0** (项目迁移): 6 个 AUTO (0.2 cp code / 0.3 cp data / 0.4 改路径 / 0.6 禁用旧位置 / 0.7 数据原位保留 / 0.8 README), 2 个 MANUAL (0.1 停 dashboard / 0.5 浏览器验证).
...
**合计**: 35 个 AUTO + 12 个 MANUAL = **47 个 task**. 人工时长约 5-8h (含 Chunk 5 长跑等待).
...
**关键路径 (依赖链)**:
```
人工准备 (5步) → Chunk 0 (8 task) → Chunk 1 (13 task, 串行)
