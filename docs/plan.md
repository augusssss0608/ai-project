...
on_wakeup():
  ...
  if not (3 <= now.hour < 10):
    ...
    if now.hour < 3:
      ...
      target = datetime.combine(now.date() + timedelta(days=1), time(3, 0), tzinfo=LOCAL_TZ)
    delta = int((target - now).total_seconds())
    ScheduleWakeup(delaySeconds=min(3600, max(60, delta)))
    return  # 本轮合法结束, 不做任何 task
...
## 预决策 (/loop 启动前用户已拍板, 遇到无需再问)

这些决策由用户在 /loop 启动前确认 (2026-04-22), /loop 撞到对应 task 时直接按决策执行, 不再写 NEED-HUMAN-INPUT:

- **Task 2.0a 失败处理**: 若 subagent `model:` 覆盖不生效 (Haiku/Opus 被主 Sonnet 覆盖), **中止项目**, 写 NEED-HUMAN-INPUT 等用户决定改架构还是放弃. 不自动降级到全 Sonnet.
- **Task 2.0b 失败处理**: 若 Agent tool 实际并发 < 10, 按实测值 N 自动调整 batch size (40 条新闻切 `ceil(40/N)` 批). 不中止, 不需用户确认.
- **Task 3.5 部分源失败**: 某源抓不到时其他源继续 (spec §14 已定). /loop 把失败源记到 raw 文件的 `error` 字段, pipeline 整体不停.
- **Task 4.6 浏览器验证方式**: 用 Playwright MCP (`browser_navigate` / `browser_snapshot` / `browser_click` / `browser_take_screenshot`) 自动验证, [AUTO] 不转 MANUAL.
- **Task 5.1 冒烟验证方式**: 主体 [AUTO] (sentinel 触发 + 文件检查 + Playwright 看 dashboard), 仅 "TG 手机是否收到" 一步 [MANUAL] 等用户回 "收到" 再继续.
- **Task 5.3 evolve 假反馈冒烟**: **跳过** (用户决定用生产真反馈自然验证; 若生产 evolve 崩溃再回来手动调).
- **Task 5.4 生产 /loop 启动方式**: 方式 A — 停掉实施 /loop, **同一个 tmux 会话**里重新输 `/loop` + 生产 prompt. 不开新 tmux.
- **Task 5.5 监控 3 条锁定指标**:
  1. TG 每天 10:00 (±30min) 收到 "ai-news 已刷新" 推送 (断推 3 天告警)
  2. `http://localhost:38080/#news` 能打开且显示当天日期数据
  3. `ai-news-history.jsonl` 每天新增 ~40 条 (< 20 视为抓取退化)
...
- [ ] **Step 1: 重启 usage-web (后台)**

```bash
# kill 旧进程 (不依赖进程名, 按端口找)
PID=$(lsof -t -i :38080 2>/dev/null) && [ -n "$PID" ] && kill "$PID" || true
sleep 1
# 后台拉起新进程
nohup python3 ~/Desktop/ai-project/hooks/usage-web.py > /tmp/usage-web.log 2>&1 &
sleep 2
# 确认启动
curl -s -o /dev/null -w "%{http_code}" http://localhost:38080 || (cat /tmp/usage-web.log; exit 1)
```
Expected: `200`.

- [ ] **Step 2: Playwright 打开首页**

调 `mcp__playwright__browser_navigate` → `http://localhost:38080`.
调 `mcp__playwright__browser_snapshot` 取得页面 accessibility tree.

Expected: snapshot 含 5 个 tab 标题文字 ("Skill" / "CLAUDE.md" / "Subagent" / "活跃项" / "每日AI大事").

- [ ] **Step 3: 切到 "每日AI大事" tab**

调 `mcp__playwright__browser_click` 点 "每日AI大事" tab.
调 `mcp__playwright__browser_snapshot` 再拿一次.

Expected:
- 原"刷新按钮"不在 snapshot 里
- 若 `ai-news.json` 存在, snapshot 含 "HN" / "GitHub" / "量子位" / "iThome" 四个源名
- snapshot 含 "阶段" 字样 + 四个源的 stage 文字 (冷/中/热 或对应文案)

- [ ] **Step 4: 验证 score tooltip 结构**

调 `mcp__playwright__browser_snapshot` 寻找每条 news item 附近的 `ai_score` DOM 元素 (Task 4.3 产出).
Expected: 每条新闻 title 旁有 score 数值展示 + reason 文本 (可能在 title 属性或同级 span).

- [ ] **Step 5: 依次切其他 4 个 tab, 每个 browser_take_screenshot**

将 screenshot 保存到 `~/Desktop/ai-project/progress/logs/task-4.6-screenshots/tab-{name}.png`.

Expected: 5 张截图都不为空 (文件 size > 10KB).

- [ ] **Step 6: 验失败时降级**

若 Step 2-5 任何一步 snapshot 不含预期关键词, 或 click 失败:
- 把当前 snapshot + screenshot 存到 `progress/logs/task-4.6-failure/`
- 写 NEED-HUMAN-INPUT.md 附上路径, 转 MANUAL (用户肉眼看截图判断问题)
...
- [ ] **Step 7: 检查 Dashboard (Playwright [AUTO])**

- `mcp__playwright__browser_navigate` → `http://localhost:38080/#news`
- `mcp__playwright__browser_snapshot`

Expected: snapshot 含当天日期 + 4 个源名 + 至少 20 条 news item; 每条含 summary / workspace_help / claude_usage / ai_score 字段文本.

失败: 把 snapshot 存 `progress/logs/task-5.1-dashboard-fail.json`, 转 MANUAL.

- [ ] **Step 8: TG 通知 [MANUAL]**

/loop 发 TG: "[Task 5.1] sentinel 冒烟已跑完, 你应该刚收到'ai-news 已刷新 N 则'推送. 收到请在 NEED-HUMAN-INPUT.md 写 `resolved: true` 并补一句 `received: true/false`".

写 `progress/NEED-HUMAN-INPUT.md`, 字段:
- `task: 5.1 step 8`
- `blocking_for: TG 推送到手机确认`
- `resolved: false`
- `received: null`

ScheduleWakeup(7200s) 挂起. 用户回 `resolved: true, received: true` → 合格通过; `received: false` → /loop 诊断 telegram MCP 调用日志, 转 AUTO_FAIL retry ≤ 3 次.

- [ ] **Step 9: 若任何一步失败**

记录问题到 `~/Desktop/ai-project/progress/logs/task-5.1-smoke-notes.md`, 排查对应 subagent 或 Python 模块.
...
### Task 5.3: [SKIPPED - 预决策 2026-04-22] Evolve 假反馈冒烟

**状态**: 跳过. 用户在 /loop 启动前决定用生产真反馈自然验证 evolve, 不做上线前假反馈冒烟.

**风险提示 (/loop 不需要执行, 仅留作记录)**:
- evolve 代码仍在 Chunk 2 的 subagent 里写死 (evolve-source-preferences)
- 生产上线后, 某源反馈累计 >= 20 才会第一次触发 evolve, 可能 1-2 周后才发生
- 如果第一次真触发 evolve 时 Opus 改写 source.md 出错 (如 YAML frontmatter 损坏), 该源后续评分会失灵, 需用户手动回滚到 `source.md.v0` 备份
- 回滚命令: `cp ~/Desktop/ai-project/.claude/skills/ai-news-filter/sources/{source_id}/source.md.v0 .../source.md`

- [ ] **Step 1: Mark complete (直接跳过此 task)**

/loop 撞到此 task 时**直接打勾**, 不执行任何步骤, 前进到 Task 5.4.
...
**锁定 3 条核心指标** (预决策 2026-04-22):

1. **TG 推送健康**: 每天 10:00 (±30min) 收到 "ai-news 已刷新" 推送. 断推连续 3 天 → 告警.
2. **Dashboard 可达性**: `http://localhost:38080/#news` 能打开, 且显示当天日期的数据.
3. **抓取量**: `ai-news-history.jsonl` 每天新增约 40 条 (4 源 × 10). 单日 < 20 视为抓取退化, 触发排查.

- [ ] **Step 1: 7 天后检查 (3 条指标 + 额外观察)**

核心指标:
```bash
# 指标 3: 近 7 天每日 new items 数
python3 <<'EOF'
import json, collections
from datetime import datetime, timedelta
path = '/Users/augus/Desktop/ai-project/data/ai-news-history.jsonl'
...
for day, n in sorted(by_day.items()):
    flag = "⚠️" if n < 20 else "✓"
    print(f"{flag} {day}: {n} items")
EOF
```

- 指标 1: 手机翻 TG 7 天记录, 数 "ai-news 已刷新" 条数 (期望 7)
- 指标 2: 打开 dashboard 看当天有无数据

额外观察:
- 反馈数: `python3 -c "import json; d=json.load(open('/Users/augus/Desktop/ai-project/data/ai-news-feedback.json')); print(f\"votes: {len(d.get('votes', {}))}\")"`
- 主观: Top 10 命中率 (你觉得是不是想看的)
...
- 是否有源进入热启动 (>= 50)? 若是, evolve 是否触发?
  - **特别检查**: evolve 触发后 `source.md` 是否正常 (因 Task 5.3 跳过, evolve 第一次真触发前没经假反馈冒烟; 发现异常立即回滚 `source.md.v0`)
...
- **Chunk 4** (Dashboard 改动): 6 个 AUTO (4.1-4.5 写代码 + 4.6 Playwright 验证). **0 个 MANUAL** (预决策后 4.6 升级).
- **Chunk 5** (集成 + 长跑 + 监控): 1 个 AUTO (5.1 主体), 3 个 MANUAL (5.1 第 8 步 TG 手机确认 / 5.2 prompt 调优 / 5.4 正式长跑 / 5.5 7/30 天监控), **1 个 SKIPPED** (5.3 evolve 假反馈冒烟, 预决策跳过).

**合计 (/loop 实际执行部分, 不含已完成的 Chunk 0)**: 31 个 AUTO + 7 个 MANUAL + 1 个 SKIPPED = **39 个 task**. 预决策 2026-04-22 把 4.6 从 MANUAL 转 AUTO (Playwright), 5.1 仅保留 TG 确认一步 MANUAL, 5.3 跳过. 人工时长缩到约 1-2h (主要是 5.2 prompt 调优 + 5.4 长跑启动 + 5.5 长期观察).
