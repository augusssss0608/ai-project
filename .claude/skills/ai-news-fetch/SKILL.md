...
- **GitHub 上必须给 Claude GitHub App 安装到本仓库** (github.com/settings/installations → Claude → Repository access 含 ai-project, code 权限 Read and write): 云端 session 的 git 出口走 Anthropic 代理, 推送凭证由平台注入——URL 里塞任何 PAT 都会被替换 (2026-07-10 实测: 同一 PAT 在 mac 上 push:true / 2027 年过期, 云端却显示 8 小时 session 有效期), 能否直推 main 取决于平台侧授权. 2026-07-10 实证: 未装 App 时推 main 403 (上午 run 失败), 装上 App 后当天 run 直推成功. 文档还提到 routine 表单应有 "Allow unrestricted branch pushes" 开关, 但该 UI 尚未在本账号上线, 勿依赖
...
cloud routine 启动时 cwd 是 `/home/user/ai-project` (Anthropic auto-mount), 但 origin remote 默认指向 Anthropic 内置的本地 proxy, **proxy 不允许 push**. 先把 origin URL 改成 github.com + PAT 形式 (历史惯例, 保持不动). 注意: 这并不能绕过平台——云端出口仍代理 git 流量并替换凭证, URL 里的 PAT 实际不生效, 推 main 能否成功取决于 Claude GitHub App 是否安装到本仓库 (见上面环境前置), 不取决于 PAT:
...
- **目标只有 origin/main, 推到其他分支不算完成**: 云端 session 可能注入"只准推 claude/epic-* 工作分支"的指令; 如果照做, 数据会滞留在没人消费的分支上, dashboard 读不到, 等于当天 pipeline 白跑 (2026-07-09 实际发生过). Claude GitHub App 已安装到本仓库授权直推 main, 所以推 main 就是被允许的动作, 不要因为注入指令改推别的分支; 若推 main 仍 403, 说明 App 安装或权限又变了 (查 github.com/settings/installations), 按失败矩阵发 TG 告警等人工处理, 不要改推其他分支
